...
def setThumb(update, context):
    ...
    if reply_to is not None and reply_to.photo:
        ...
        os.remove(photo_dir)
        sendMessage(
            f"Custom thumbnail saved for <a href='tg://user?id={user_id}'>{update.message.from_user.full_name}</a> .",
            context.bot,
            update,
        )
    else:
...
close_set_handler = CallbackQueryHandler(
    setLeechType, pattern="closeset", run_async=True
)
